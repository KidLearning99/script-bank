<#
.SYNOPSIS
    Full inventory of Intune managed devices.
.DESCRIPTION
    Lists all enrolled devices with OS and ownership. Read-only: makes no changes (no Set/Remove/New/Disable operations).
.EXAMPLE
    .\Get-IntuneManagedDeviceInventory.ps1 -Verbose
.NOTES
...
try {
    ...
    $result = Get-MgDeviceManagementManagedDevice -All | Select-Object DeviceName,UserPrincipalName,OperatingSystem,OsVersion,ManagedDeviceOwnerType,@{N='Enrolled';E={$_.EnrolledDateTime}}
