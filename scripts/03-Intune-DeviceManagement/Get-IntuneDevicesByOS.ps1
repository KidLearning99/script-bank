<#
.SYNOPSIS
    Count of enrolled devices grouped by OS.
.DESCRIPTION
    Summarises the device fleet by operating system. Read-only: makes no changes (no Set/Remove/New/Disable operations).
.EXAMPLE
    .\Get-IntuneDevicesByOS.ps1 -Verbose
.NOTES
...
try {
    ...
    $result = Get-MgDeviceManagementManagedDevice -All | Group-Object OperatingSystem | Select-Object Name,Count
