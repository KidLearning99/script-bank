<#
.SYNOPSIS
    Device encryption (BitLocker/FileVault) state.
.DESCRIPTION
    Reports per-device encryption status. Read-only: makes no changes (no Set/Remove/New/Disable operations).
.EXAMPLE
    .\Get-IntuneEncryptionReport.ps1 -Verbose
.NOTES
...
try {
    ...
    $result = Get-MgDeviceManagementManagedDevice -All | Select-Object DeviceName,UserPrincipalName,OperatingSystem,@{N='Encrypted';E={$_.IsEncrypted}}
