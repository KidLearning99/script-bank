...
.DESCRIPTION
    Flags managed devices with old last-sync dates. Read-only: makes no changes (no Set/Remove/New/Disable operations).
.EXAMPLE
    .\Get-IntuneStaleDevices.ps1 -Verbose
.NOTES
...
try {
    ...
    $result = $c=(Get-Date).AddDays(-30); Get-MgDeviceManagementManagedDevice -All | Where-Object {$_.LastSyncDateTime -lt $c} | Select-Object DeviceName,UserPrincipalName,@{N='LastSync';E={$_.LastSyncDateTime}}
